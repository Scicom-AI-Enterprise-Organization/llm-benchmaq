"""
Configuration dataclasses for benchmaxxing.

All dataclasses are picklable for multiprocessing compatibility.
"""
...
class ParallelismConfig:
    """Parallelism configuration for vLLM server."""
...
class VLLMServeConfig:
    ...
    parallelism_pairs: List[ParallelismConfig] = field(default_factory=list)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "parallelism_pairs": [
                {"tensor_parallel": p.tensor_parallel, "data_parallel": p.data_parallel, "pipeline_parallel": p.pipeline_parallel}
                for p in self.parallelism_pairs
            ],
        }
...
class RemoteConfig:
    ...
    dependencies: List[str] = field(default_factory=lambda: [
        "pyyaml", "requests", "vllm==0.11.0", "huggingface_hub"
    ])
...
@dataclass
class RunPodContainerConfig:
    """Configuration for RunPod container."""
    image: str = "runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04"
    disk_size: int = 20


@dataclass
class RunPodStorageConfig:
    """Configuration for RunPod storage."""
    volume_size: int = 100
...
@dataclass
class RunPodConfig:
    """Configuration for RunPod deployment and benchmarking."""
    api_key: Optional[str] = None
    ssh_private_key: Optional[str] = None
    pod: RunPodPodConfig = field(default_factory=RunPodPodConfig)
    container: RunPodContainerConfig = field(default_factory=RunPodContainerConfig)
    storage: RunPodStorageConfig = field(default_factory=RunPodStorageConfig)
    ports_http: List[int] = field(default_factory=lambda: [8888, 8000])
    ports_tcp: List[int] = field(default_factory=lambda: [22])
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "pod": {
                "name": self.pod.name,
                "gpu_type": self.pod.gpu_type,
                "gpu_count": self.pod.gpu_count,
                "instance_type": self.pod.instance_type,
                "bid_per_gpu": self.pod.bid_per_gpu,
                "secure_cloud": self.pod.secure_cloud,
            },
            "container": {
            ...
            "storage": {
                "volume_size": self.storage.volume_size,
                "mount_path": self.storage.mount_path,
            },
            "ports": {
                "http": self.ports_http,
                "tcp": self.ports_tcp,
            },
            "env": self.env,
        }
...
class BenchConfig:
    """
    Main configuration for benchmaxxing.
    
    Supports both local and remote execution.
    All fields are picklable for multiprocessing compatibility.
    """
    # Run identification
    name: str = "benchmark"
    engine: str = "vllm"
    ...
    def to_run_config(self) -> dict:
        """Convert to a run config dict compatible with runner.py"""
        return {
            "name": self.name,
            "engine": self.engine,
            "model": {
    ...
    def from_dict(cls, d: dict) -> "BenchConfig":
        """Create BenchConfig from a flat dict of kwargs."""
...
def kwargs_to_run_config(**kwargs) -> dict:
    ...
    Supports shortcuts like tensor_parallel -> vllm_serve.parallelism_pairs
    """
    config = {
        "runs": [{
